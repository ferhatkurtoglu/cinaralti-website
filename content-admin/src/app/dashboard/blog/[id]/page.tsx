...
export default function BlogPostPage({ params }: { params: { id: string } }) {
  ...
  // Otomatik slug oluşturma (sadece yeni yazılar için)
  useEffect(() => {
    if (params.id === 'new' && watchedTitle) {
  ...
  }, [watchedTitle, setValue, params.id])
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (formData.title && formData.content) {
        const key = params.id === 'new' ? 'blog-draft' : `blog-draft-${params.id}`
  ...
  }, [watch, autoSaveEnabled, params.id])

  const onSubmit = async (data: FormData) => {
    const loadingToast = toast.loading(
      params.id === 'new' ? 'Blog yazısı kaydediliyor...' : 'Blog yazısı güncelleniyor...'
    )
    ...
    try {
      ...
      toast.success(
        params.id === 'new' ? 'Blog yazısı başarıyla kaydedildi!' : 'Blog yazısı başarıyla güncellendi!',
        { id: loadingToast }
      )
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs text-gray-500">
                      Yazınızın URL'sinde görünecek olan slug. 
                      {params.id === 'new' && ' Başlık girildiğinde otomatik oluşturulur.'}
                    </p>
                  </div>
                </div>
              </div>
                ...
                >
                  {isLoading ? (
                    <>
                      ...
                      {params.id === 'new' ? 'Kaydediliyor...' : 'Güncelleniyor...'}
                    </>
                  ) : (
                    <>
                      <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      {params.id === 'new' ? 'Yayınla' : 'Güncelle'}
                    </>
                  )}
                </button>
              </div>
